 ...
 *
 * It runs the checkout's committed .tokens-per-ticket/tpt.mjs, as husky and
 * .claude/settings.json hooks run committed code: review changes to both.
 */
...
cli="$(git rev-parse --show-toplevel 2>/dev/null)/${BUNDLE_PATH}"
[ -f "$cli" ] || exit 0
